...
namespace view {
VaapiTexturesSrb::VaapiTexturesSrb(QRhi* rhi, Uniforms* uniforms, const codec::FramePtr& frame)
    : m_rhi(rhi)
    , m_vaDisplay(frame->vaDisplay())
    , m_width(frame->width())
    ...
    , m_uniforms(uniforms)
{
    ...
    if (m_rhi->backend() != QRhi::OpenGLES2) {
        LOGE("VaapiTexturesSrb only supports OpenGLES2 backend.");
        throw std::runtime_error("VaapiTexturesSrb only supports OpenGLES2 backend.");
    }

    if (!m_vaDisplay) {
        LOGE("Invalid VA display.");
        throw std::runtime_error("Invalid VA display.");
    }

    m_eglDisplay = eglGetCurrentDisplay();
    if (m_eglDisplay == EGL_NO_DISPLAY) {
        LOGE("Failed to get current EGL display.");
        throw std::runtime_error("Failed to get current EGL display.");
    }

    if (s_eglCreateImageKHR == nullptr ||
        s_eglDestroyImageKHR == nullptr ||
        s_glEGLImageTargetTexture2DOES == nullptr) {
        LOGE("Failed to get EGL extension function pointers.");
    ...
    if (m_yTexture->create() == false || m_uvTexture->create() == false) {
        LOGE("Failed to create QRhiTextures for VAAPI SRB creation");
        m_yTexture.reset();
        m_uvTexture.reset();
        throw std::runtime_error("Failed to create QRhiTextures for VAAPI SRB creation");
    }
    ...
    m_srb->setBindings({
        ...
        QRhiShaderResourceBinding::uniformBuffer(3, QRhiShaderResourceBinding::VertexStage, m_uniforms->vsUBuffer()),
        QRhiShaderResourceBinding::uniformBuffer(4, QRhiShaderResourceBinding::FragmentStage, m_uniforms->colorParamsUBuffer()),
    });
    if (!m_srb->create()) {
        LOGE("Failed to create shader resource bindings");
        m_srb.reset();
        throw std::runtime_error("Failed to create shader resource bindings");
    }
}

VaapiTexturesSrb::~VaapiTexturesSrb()
{
    FUNC_TRACE;
}

QString VaapiTexturesSrb::getFragmentShaderName()
{
    return ":/shaders/nv12.frag.qsb";
}
void VaapiTexturesSrb::updateTexture(QRhiResourceUpdateBatch* rub, const codec::FramePtr& frame)
{
    VASurfaceID vaSurface = static_cast<VASurfaceID>(reinterpret_cast<uintptr_t>(frame->data(3)));
...
int VaapiTexturesSrb::yGLTexture() const
{
    return static_cast<GLint>(m_yTexture->nativeTexture().object);
}
int VaapiTexturesSrb::uvGLTexture() const
{
